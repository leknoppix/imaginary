...
RUN ldd /go/bin/imaginary | tr -s '[:blank:]' '\n' | grep '^/' | \
    ...
    && rm -rf /go/src/github.com/sycured/imaginary/deps/lib \
    && rm /go/src/github.com/sycured/imaginary/deps/usr/lib/libc.so.* \
    && rm /go/src/github.com/sycured/imaginary/deps/usr/lib/libm.so.* \
